...
function _typeof(a){
    ...
    var storage=new ArrayStorage("tasks"),tasks=storage.list;// Une fonction qui ajoute les tâches au DOM avec un bouton de suppression auquel on attache un évènement
function taskToDOM(a){// Si on a une chaîne non-vide
    if("string"==typeof a&&a){
        ...
        c.addEventListener("click",function(){
            var a=c.parentNode.firstChild.textContent;
            storage.remove(a),
...
function newTask(){
    -1===storage.list.indexOf(input.value)&&taskToDOM(input.value)&&(storage.set(input.value),input.value=""),
    input.focus()
...
load.addEventListener("click",function(){
    fetch(url.value).then(function(a){
        if(a.ok)
        return a.json();
        throw new Error("".concat(a.statusText," (").concat(a.status,")"))
    }).then(function(a){
            if(Array.isArray(a))
            return void a.forEach(function(a) {
                -1===storage.list.indexOf(a)&&taskToDOM(a)&&storage.set(a)
            })
